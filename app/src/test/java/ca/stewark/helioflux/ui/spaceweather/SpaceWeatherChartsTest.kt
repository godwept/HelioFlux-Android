...
class SpaceWeatherChartsTest{
 ...
 @Test fun chartMetadataMatchesApprovedHierarchy(){
  assertEquals(ChartCardMeta("Magnetic Field","IMF Bz / Bt (nT)",listOf("Bz","Bt")),bzBtChartMeta)
  assertEquals("Solar Wind",densityChartMeta.context);assertEquals("Density (p/cm³)",densityChartMeta.title)
  assertEquals("Solar Wind",speedChartMeta.context);assertEquals("Speed (km/s)",speedChartMeta.title)
  assertEquals("Solar Wind",temperatureChartMeta.context);assertEquals("Temperature (K)",temperatureChartMeta.title)
  assertEquals(ChartCardMeta("GOES Magnetometer","Magnetic Field (nT)",listOf("GOES-19","GOES-18")),goesChartMeta("GOES-19","GOES-18"))
  assertEquals(emptyList<String>(),goesChartMeta(null,null).legendLabels)
 }
 @Test fun hemisphericPowerHasNorthAndSouthSeries(){val out=hemisphericPowerSeries(listOf(HemisphericPowerSample(now-1,40.0,30.0)),Timeframe.OneHour,now);assertEquals(40.0,out[0].points.single().y!!,0.0);assertEquals(30.0,out[1].points.single().y!!,0.0);assertEquals(listOf("North","South"),hemisphericPowerMeta.legendLabels)}
 @Test fun missingGoesDoesNotFabricateSeries(){assertTrue(goesSeriesOrEmpty(null,Timeframe.OneHour,now).isEmpty())}
 @Test fun hemisphericEmptyStateContractIsStable(){assertEquals("No hemispheric power data",hemisphericPowerEmptyMessage)}
}
