...
fun kpPresentation(samples:List<KpSample>,timeframe:Timeframe,now:Long):KpPresentation{val filtered=filterByTimeframe(samples,timeframe,now){it.timestampMillis}.mapNotNull{s->s.kp?.let{s.timestampMillis.toDouble() to it}};return KpPresentation(filtered,filtered.lastOrNull()?.second?.let(::kpStatus))}
