...
@Composable fun SpaceWeatherScreen(state:SpaceWeatherUiState,expanded:Boolean,onTimeframe:(Timeframe)->Unit,modifier:Modifier=Modifier,nowMillis:Long=System.currentTimeMillis()){
 val magnetic=state.magnetic.screenData().orEmpty();val plasma=state.plasma.screenData().orEmpty();val kp=state.kp.screenData().orEmpty();val goes=state.goesMagnetometer.screenData();val hp=state.hemisphericPower.screenData().orEmpty()
 ...
 LazyColumn(modifier.testTag("space-weather-screen").padding(16.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){
  ...
  item{Card(Modifier.fillMaxWidth().height(240.dp).testTag("aurora-globe-slot")){Box(Modifier.fillMaxSize()){Text("Aurora Globe",Modifier.padding(16.dp))}}}
