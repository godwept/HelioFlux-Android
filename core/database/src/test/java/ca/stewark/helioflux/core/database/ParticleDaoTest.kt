...
class ParticleDaoTest {
 @Test fun particleSeriesPersistNullableChannelsAndQuerySeventyTwoHours() = runBlocking {
  val db=inMemoryDatabase(); try {
   ...
   val ep=db.aceEpamDao().getBetween(now-72*hour,now); assertEquals(2,ep.size); assertEquals(null,ep.first().electronLow); assertEquals(7.0,ep.first().protonP7!!,0.0); assertEquals(9.0,ep.last().protonP8!!,0.0)
