...
class ParticleDaoTest {
 @Test fun particleSeriesPersistNullableChannelsAndQuerySeventyTwoHours() = runBlocking {
  val db=inMemoryDatabase(); try {
   ...
   db.aceEpamDao().upsertAll(listOf(AceEpamEntity(now-72*hour,null,2.0,3.0,null,5.0),AceEpamEntity(now,1.0,null,null,4.0,null)))
   val ep=db.aceEpamDao().getBetween(now-72*hour,now); assertEquals(2,ep.size); assertEquals(null,ep.first().electronLow); assertEquals(null,ep.last().protonHigh)
