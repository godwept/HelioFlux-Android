...
class SolarHeroRepositoryTest {
 ...
 @Test fun refreshReplacesCachedFramesWithoutPublishingAnEmptySet()=runTest{val now=2_000_000_000L;val dao=Frames(listOf(SolarHeroFrameEntity("old",1000,"old")));val status=Status();status.upsert(DataSourceStatusEntity(SolarHeroRepository.SOURCE,1000,1000,1000,1000,null,null));val repo=SolarHeroRepository(dao,status,HelioviewerApi(HeroHttp(now))){now};repo.refresh();assertFalse(dao.history.any{it.isEmpty()});assertTrue(dao.flow.value.isNotEmpty())}
 ...
 private class Frames(seed:List<SolarHeroFrameEntity>):SolarHeroFrameDao{val flow=MutableStateFlow(seed);val history=mutableListOf(seed);override suspend fun upsertAll(frames:List<SolarHeroFrameEntity>){history+=listOf(frames);flow.value=frames};override fun observeAll():Flow<List<SolarHeroFrameEntity>> = flow;override suspend fun latest()=flow.value.maxByOrNull{it.sourceTimestampMillis};override suspend fun deleteAll(){history+=listOf(emptyList());flow.value=emptyList()};override suspend fun replaceAll(frames:List<SolarHeroFrameEntity>){history+=listOf(frames);flow.value=frames}}
