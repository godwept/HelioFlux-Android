...
class SolarHeroRepositoryTest {
 ...
 @Test fun widgetLatestRefreshDoesNotPopulateAnimationFrameCache()=runTest{val now=2_000_000_000L;val dao=Frames(emptyList());val status=Status();val repo=SolarHeroRepository(dao,status,HelioviewerApi(HeroHttp(now))){now};val image=repo.refreshLatest();assertNotNull(image);assertTrue("widget latest refresh must not populate the Home animation cache",dao.flow.value.isEmpty())}
 @Test fun normalRefreshProducesOrderedMultiFrameAnimationSet()=runTest{val now=Instant.parse("2026-09-20T16:00:00Z").toEpochMilli();val dao=Frames(emptyList());val status=Status();val http=HistoricalHeroHttp();val repo=SolarHeroRepository(dao,status,HelioviewerApi(http)){now};repo.refresh();val rows=(repo.frames().first() as RepositoryState.Available).data;assertEquals(60,http.requestedDates.size);assertEquals(60,rows.size);assertEquals(60,rows.map{it.url}.distinct().size);assertEquals(rows.mapNotNull{it.sourceTimestampMillis}.sorted(),rows.mapNotNull{it.sourceTimestampMillis});val requested=http.requestedDates.sorted();assertEquals(Instant.ofEpochMilli(now-59*15*60_000L),requested.first());assertEquals(Instant.ofEpochMilli(now),requested.last())}
 @Test fun latestOnlyRefreshLeavesAnimationCacheEmptyAndFullRefreshStillRuns()=runTest{val now=Instant.parse("2026-09-20T16:00:00Z").toEpochMilli();val dao=Frames(emptyList());val status=Status();val http=HistoricalHeroHttp();val repo=SolarHeroRepository(dao,status,HelioviewerApi(http)){now};assertNotNull(repo.refreshLatest());assertTrue(dao.flow.value.isEmpty());repo.refreshIfStale();assertEquals(60,dao.flow.value.size);assertEquals(61,http.requestedDates.size)}
 ...
 @Test fun refreshLatestDoesNotModifyExistingAnimationFrames()=runTest{val now=2_000_000_000L;val dao=Frames(listOf(SolarHeroFrameEntity("old",now-1000,"old")));val status=Status();val repo=SolarHeroRepository(dao,status,HelioviewerApi(HeroHttp(now))){now};assertNotNull(repo.refreshLatest());assertEquals(listOf("old"),dao.flow.value.map{it.imageId})}
 ...
 private class HistoricalHeroHttp:HttpTransport{
  val requestedDates=Collections.synchronizedList(mutableListOf<Instant>())
  override suspend fun get(url:String):HttpResult{
