...
class SolarImageryRepositoryTest {
 private val now=Instant.parse("2026-09-22T11:20:00Z").toEpochMilli()
 private val hmiMetadataUrl=HelioFluxEndpoints.hmiMetadata(now-24*60*60*1000L,now)
 private val hekUrl=HelioFluxEndpoints.hek(now-24*60*60*1000L,now)
 private val hmibcUrl="https://iswa.ccmc.gsfc.nasa.gov/iswa_data_tree/observation/solar/sdo/hmi-magnetogram-color_2048x2048/2026/09/20260922_111500_2048_HMIBC.jpg"
 private val hmibcJson="""{"iswa_sdo_aia_hmic_files":{"samples":[{"timestamp":"2026-09-22 11:15:00.0","url":"$hmibcUrl"}]}}"""
 private val activeRegionJson="""{"result":[{"ar_noaanum":14532,"hpc_x":10.0,"hpc_y":20.0}]}"""
 @Test fun sourcesPersistIndependentlyWhenOneFails()=runTest{val f=Fakes();val r=f.repo();f.http.getStatus[hmiMetadataUrl]=500;f.http.getBody[hekUrl]=activeRegionJson;f.http.getBody[HelioFluxEndpoints.enlil]="""<a href="enlil_com2_1_20260919T120000.jpg">x</a>""";r.refreshAll();assertTrue(r.image(SolarImageType.Magnetogram).first() is RepositoryState.Failure);assertEquals(1,(r.regions().first() as RepositoryState.Available).data.size);assertEquals(1,(r.enlil().first() as RepositoryState.Available).data.size)}
 ...
 @Test fun magnetogramRefreshPersistsLatestLaTiSHmibcSample()=runTest{val f=Fakes();val r=f.repo();f.http.getBody[hmiMetadataUrl]=hmibcJson;r.refreshMagnetogram();val image=(r.image(SolarImageType.Magnetogram).first() as RepositoryState.Available).data;val timestamp=Instant.parse("2026-09-22T11:15:00Z").toEpochMilli();assertEquals(timestamp,image.sourceTimestampMillis);assertEquals(hmibcUrl,image.url);assertEquals(timestamp,f.status.get(SolarImageryRepository.HMI)?.observationTimestampMillis)}
 ...
 @Test fun refreshRegionsUsesBoundedHekSearchAndPublishesLabels()=runTest{val f=Fakes();val r=f.repo();f.http.getBody[hekUrl]=activeRegionJson;r.refreshRegions();assertEquals(listOf(hekUrl),f.http.requestedGets);val state=r.regions().first() as RepositoryState.Available;assertEquals(1,state.data.size);assertEquals("4532",state.data.single().number)}
 private class Fakes{val image=Images();val region=Regions();val enlil=Enlil();val status=Status();val http=Http();fun repo()=SolarImageryRepository(image,region,enlil,status,http){Instant.parse("2026-09-22T11:20:00Z").toEpochMilli()}}
