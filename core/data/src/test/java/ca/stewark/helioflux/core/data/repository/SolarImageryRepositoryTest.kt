...
None"""
 ...
 @Test fun refreshAllUsesWorkingLascoAnimationEndpoints()=runTest{val f=Fakes();val r=f.repo();f.http.getBody[hmiMetadataUrl]=hmibcJson;r.refreshAll();assertNotNull(f.image.m[SolarImageType.LascoC2]?.value);assertEquals(HelioFluxEndpoints.lasco+"LATEST/current_c2small.gif",f.image.m[SolarImageType.LascoC2]?.value?.url);assertEquals(HelioFluxEndpoints.lasco+"LATEST/current_c3small.gif",f.image.m[SolarImageType.LascoC3]?.value?.url)}
 @Test fun headLastModifiedBecomesNormalizedTimestamp()=runTest{val f=Fakes();val r=f.repo();val url=HelioFluxEndpoints.lasco+"LATEST/current_c2small.gif";f.http.headers[url]=mapOf("Last-Modified" to listOf("Sat, 19 Sep 2026 12:00:00 GMT"));r.refreshImage(SolarImageType.LascoC2,url);assertEquals(1789819200000L,(r.image(SolarImageType.LascoC2).first() as RepositoryState.Available).data.sourceTimestampMillis)}
