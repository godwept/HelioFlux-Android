...
import ca.stewark.helioflux.core.data.parser.SolarRegionSummaryParser
...
class SolarImageryRepository(private val imageDao:SolarImageDao,private val regionDao:ActiveRegionDao,private val enlilDao:EnlilFrameDao,private val statusDao:DataSourceStatusDao,private val transport:HttpTransport,private val nowMillis:()->Long=System::currentTimeMillis){
 ...
 suspend fun refreshAll(){refreshMagnetogram();refreshImage(SolarImageType.LascoC2,HelioFluxEndpoints.lasco+"LATEST/current_c2small.gif");refreshImage(SolarImageType.LascoC3,HelioFluxEndpoints.lasco+"LATEST/current_c3small.gif");refreshRegions();refreshEnlil()}
 ...
 suspend fun refreshRegions(){val now=nowMillis();attempt(HEK,now);try{val target=imageDao.getLatest(SolarImageType.Magnetogram)?.sourceTimestampMillis?:now;val r=transport.get(HelioFluxEndpoints.solarRegionSummary);require(r.status in 200..299){"HTTP "+r.status};val rows=SolarRegionSummaryParser.parse(r.body,target);require(rows.isNotEmpty()){"SRS contained no current regions"};regionDao.replaceAll(rows.map(ActiveRegion::toEntity));success(HEK,target,now)}catch(e:Exception){fail(HEK,now,e)}}
 ...
 companion object{private const val HMI_LOOKBACK_MILLIS=24*60*60*1000L;val HMI=DataSourceKey("worker-hmi");val LASCO_C2=DataSourceKey("worker-lasco-c2");val LASCO_C3=DataSourceKey("worker-lasco-c3");val HEK=DataSourceKey("worker-hek");val ENLIL=DataSourceKey("worker-enlil")}
}
