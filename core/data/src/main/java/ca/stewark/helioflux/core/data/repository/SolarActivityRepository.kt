...
class SolarActivityRepository(private val xrayDao:XrayFluxDao,private val flareDao:FlareEventDao,private val cmeDao:CmeEventDao,private val aceEpamDao:AceEpamDao,private val statusDao:DataSourceStatusDao,private val transport:HttpTransport,private val nowMillis:()->Long=System::currentTimeMillis){
 ...
 fun xray(a:Long,b:Long)=combine(xrayDao.observeBetween(a,b),statusDao.observe(XRAY)){r,s->state(r.map{it.toDomain()},r.isNotEmpty(),s,XRAY,FreshnessSource.REALTIME_NOAA)}
 ...
 fun aceEpam(a:Long,b:Long)=combine(aceEpamDao.observeBetween(a,b),statusDao.observe(ACE_EPAM)){r,s->state(r.map{it.toDomain()},r.isNotEmpty(),s,ACE_EPAM,FreshnessSource.REALTIME_NOAA)}
 ...
 suspend fun refreshXray(){val n=nowMillis();attempt(XRAY,n);try{val all=XrayFluxParser.parse(transport.get(HelioFluxEndpoints.goesPrimaryXray).body())+XrayFluxParser.parse(transport.get(HelioFluxEndpoints.goesSecondaryXray).body());val p=all.groupBy{it.timestampMillis}.map{(t,v)->XrayFluxSample(t,v.firstNotNullOfOrNull{it.goes18Short},v.firstNotNullOfOrNull{it.goes18Long},v.firstNotNullOfOrNull{it.goes19Short},v.firstNotNullOfOrNull{it.goes19Long})}.filter{it.timestampMillis>=n-WINDOW}.sortedBy{it.timestampMillis};require(p.isNotEmpty()){"GOES X-ray response contained no valid rows"};xrayDao.upsertAll(p.map(XrayFluxSample::toEntity));success(XRAY,p.maxOf{it.timestampMillis},n)}catch(e:Exception){fail(XRAY,n,e)}}
 suspend fun refreshFlares()=donki(DONKI_FLARES,"FLR",parse=DonkiFlareParser::parse,persist={flareDao.upsertAll(it.map(FlareEvent::toEntity))},obs={it.maxOfOrNull(FlareEvent::timestampMillis)})
 suspend fun refreshCmes()=donki(DONKI_CMES,"CMEAnalysis","&mostAccurateOnly=true",DonkiCmeParser::parse,{cmeDao.upsertAll(it.map(CmeEvent::toEntity))},{it.maxOfOrNull(CmeEvent::timestampMillis)})
 suspend fun refreshAceEpam(){val n=nowMillis();attempt(ACE_EPAM,n);try{val p=AceEpamParser.last72Hours(AceEpamParser.parse(transport.get(HelioFluxEndpoints.aceEpam).body()),n);require(p.isNotEmpty()){"ACE EPAM response contained no valid rows"};aceEpamDao.upsertAll(p.map(AceEpamSample::toEntity));success(ACE_EPAM,p.maxOf{it.timestampMillis},n)}catch(e:Exception){fail(ACE_EPAM,n,e)}}
